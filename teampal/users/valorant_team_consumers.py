# users/valorant_team_consumers.py
...
class valorant_TeamSearchConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_group_name = "valorant_team_search"
    ...
    def get_teams(self):
        return list(valorant_Team.objects.values('name', 'description', 'game', 'members_needed', 'contact', 'creator'))
    ...
    def save_team(self, name, description, game, members_needed, contact, creator):
        try:
            team = valorant_Team.objects.create(name=name, description=description, game=game, members_needed=members_needed, contact=contact, creator=creator)
    ...
    def cancel_team(self, team_name):
        try:
            team = valorant_Team.objects.get(name=team_name)
            team.delete()
            logger.info(f"Team {team_name} cancelled successfully.")
        except valorant_Team.DoesNotExist:
