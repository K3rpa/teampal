...
from .models import cs2_TournamentInterest, cs2_TournamentPost
import logging
...
class cs2_TourConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_group_name = "cs2_tournament_search"
    ...
    def get_tournaments(self):
        ...
        tournaments = cs2_TournamentPost.objects.annotate(creator_email=F('creator__email')).values(
    ...
    async def create_tournament(self, data):
        ...
        try:
            ...
            tournament = await database_sync_to_async(cs2_TournamentPost.objects.create)(
                name=data['name'],
                game=data['game'],
                date=data['date'],
    ...
    def cancel_tournament(self, name):
        try:
            tournament = cs2_TournamentPost.objects.get(name=name)
            tournament.delete()
            logger.info(f"Tournament {name} cancelled successfully.")
        except cs2_TournamentPost.DoesNotExist:
    ...
    def toggle_interest(self, user_email, tournament_name):
        ...
        try:
            ...
            tournament = cs2_TournamentPost.objects.get(name=tournament_name)
            obj, created = cs2_TournamentInterest.objects.get_or_create(
                user=user,
            ...
            current_interest_count = cs2_TournamentInterest.objects.filter(
