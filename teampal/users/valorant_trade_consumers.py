...
import json
import logging
from .models import valorant_Trade, valorant_Offer
logger = logging.getLogger(__name__)
class valorant_TradeConsumer(AsyncWebsocketConsumer):
    ...
    def get_trades(self):
        return list(valorant_Trade.objects.values(
    ...
    def get_trade_offers(self, trade_id):
        offers = valorant_Offer.objects.filter(trade_id=trade_id).order_by('-created_at').values(
    ...
    def cancel_trade(self, trade_id):
        try:
            trade = valorant_Trade.objects.get(id=trade_id)
            trade.delete()
            logger.info(f"Trade {trade_id} cancelled successfully.")
        except valorant_Trade.DoesNotExist:
    ...
    async def create_or_update_offer(self, item_name, email, offer_price, offer_quantity):
        ...
        try:
            trade = await database_sync_to_async(valorant_Trade.objects.get)(item_name=item_name)
            offer, created = await database_sync_to_async(valorant_Offer.objects.update_or_create)(
                trade=trade,
                user=user,
    ...
    async def create_trade(self, data, creator_email):
        ...
        trade = await database_sync_to_async(valorant_Trade.objects.create)(
