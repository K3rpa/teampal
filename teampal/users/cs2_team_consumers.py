# users/cs2_team_consumers.py

from channels.generic.websocket import AsyncWebsocketConsumer
from channels.db import database_sync_to_async
import json
from django.core.exceptions import ObjectDoesNotExist
from .models import cs2_Team
import logging
...
class cs2_TeamSearchConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_group_name = "cs2_team_search"
    ...
    def get_teams(self):
        return list(cs2_Team.objects.values('name', 'description', 'game', 'members_needed', 'contact', 'creator'))
    ...
    def save_team(self, name, description, game, members_needed, contact, creator):
        try:
            team = cs2_Team.objects.create(name=name, description=description, game=game, members_needed=members_needed, contact=contact, creator=creator)
    ...
    def cancel_team(self, team_name):
        try:
            team = cs2_Team.objects.get(name=team_name)
            team.delete()
            logger.info(f"Team {team_name} cancelled successfully.")
        except cs2_Team.DoesNotExist:
