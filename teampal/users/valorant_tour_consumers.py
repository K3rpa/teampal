...
from .models import valorant_TournamentInterest, valorant_TournamentPost
import logging
...
class valorant_TourConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_group_name = "valorant_tournament_search"
    ...
    def get_tournaments(self):
        ...
        tournaments = valorant_TournamentPost.objects.annotate(creator_email=F('creator__email')).values(
    ...
    async def create_tournament(self, data):
        ...
        try:
            ...
            tournament = await database_sync_to_async(valorant_TournamentPost.objects.create)(
                name=data['name'],
                game=data['game'],
                date=data['date'],
    ...
    def cancel_tournament(self, name):
        try:
            tournament = valorant_TournamentPost.objects.get(name=name)
            tournament.delete()
            logger.info(f"Tournament {name} cancelled successfully.")
        except valorant_TournamentPost.DoesNotExist:
    ...
    def toggle_interest(self, user_email, tournament_name):
        ...
        try:
            ...
            tournament = valorant_TournamentPost.objects.get(name=tournament_name)
            obj, created = valorant_TournamentInterest.objects.get_or_create(
                user=user,
            ...
            current_interest_count = valorant_TournamentInterest.objects.filter(
